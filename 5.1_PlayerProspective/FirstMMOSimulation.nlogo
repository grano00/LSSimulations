...
;;TO FIX - if the after the first pass, does it not find enough players?
  let players []
...
to create-players [p n]
  ...
  ;;Generate and shuffle a list of agents
  set turtle-list n-values p [ [i] -> i ]
  let shuffled-turtle-list []
  set shuffled-turtle-list shuffle turtle-list

  ;;Assign to the firts n player the value of newbie
  foreach n-values n [[i] -> i][
    [x] ->
    ask turtle item x shuffled-turtle-list [
